...
let make_credentials uuid n =
  ...
  Belenios_tool_common.Tool_credgen.(
    ...
    credentials.public, (List.map (fun (_a, b) -> b) credentials.priv)
  )
...
let makeCredentials (uuid:string) (n:int) =
  ...
  (Array.of_list(public_creds), Array.of_list(private_creds))
...
let decrypt
  ...
  (privkey:string)
  =
  let (a, b) = decrypt election (Array.to_list ballots) trustees (Array.to_list credentials) privkey in
  (a, b)
...
let result
  ...
  let res = compute_result election (Array.to_list ballots) trustees (Array.to_list credentials) partial_decryptions in
  res
