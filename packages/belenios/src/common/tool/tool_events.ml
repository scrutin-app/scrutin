...
module IoReader = struct
  ...
  let read_block ic buffer =
    really_input ic buffer 0 block_size
end
...
let get_data i x =
  gethash ~index:i.map ~filename:i.file x

let get_event i x =
  gethash ~index:i.map ~filename:i.file x
  |> Option.map event_of_string
...
let fsck index =
  ...
  let module IoArchiver =
    struct
      include DirectMonad
      let get_hash hash =
        gethash ~index:index.map ~filename:index.file hash
    end
  in
...
let write_header filename ~timestamp =
  let oc = open_out_gen [Open_wronly; Open_append; Open_creat; Open_binary] 0o644 filename in
  Fun.protect
    ~finally:(fun () -> close_out oc)
    (fun () ->
      Writer.write_header oc {version = 1; timestamp}
    )

let raw_append ~filename ~timestamp xs =
  let oc = open_out_gen [Open_wronly; Open_append; Open_binary] 0o644 filename in
  Fun.protect
    ~finally:(fun () -> close_out oc)
    (fun () ->
      List.fold_left
        (fun accu (typ, x) ->
          Writer.write_record oc ~timestamp typ x :: accu
        ) [] xs
    )
...
let append index ops =
  let last_event, roots, items, lines =
    List.fold_left
      (fun (last_event, roots, items, lines) x ->
        ...
        | Event (event_typ, event_payload) ->
           let event_parent, event_height =
             ...
             | None -> None, 0
             | Some x -> Some (Hash.hash_string (string_of_event x)), x.event_height + 1
           in
           let event = {event_parent; event_height; event_typ; event_payload} in
      ...
      ) (index.last_event, index.roots, [], index.lines) ops
  in
  let items = List.rev items in
  let records = raw_append ~filename:index.file ~timestamp:index.timestamp items in
